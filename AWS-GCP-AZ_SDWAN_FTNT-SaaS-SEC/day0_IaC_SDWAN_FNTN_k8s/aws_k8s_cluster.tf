...
resource "aws_network_interface" "node_master_ni" {
  ...
  private_ips       = [cidrhost(local.nodes_subnet_cidr, local.node_master_cidrhost)]
...
resource "aws_network_interface" "node_worker_ni" {
  ...
  subnet_id         = local.nodes_subnet_id
  security_groups   = [local.nodes_sg_id]
  private_ips       = [cidrhost(local.nodes_subnet_cidr, local.node_master_cidrhost + count.index + 1)]
  source_dest_check = false
  tags = {
...
data "template_file" "node_master" {
  ...
  vars = {
    cert_extra_sans = module.fgt.fgt_active_eip_public
    script          = data.template_file.node_master_script.rendered
    k8s_version     = local.k8s_version
    forticnp_token  = var.forticnp_token
  }
}
data "template_file" "node_master_script" {
  depends_on = [module.fgt]
...
data "template_file" "node_worker" {
  ...
  vars = {
    script      = data.template_file.node_workder_script.rendered
    k8s_version = local.k8s_version
  }
}
data "template_file" "node_workder_script" {
  depends_on = [module.fgt]
